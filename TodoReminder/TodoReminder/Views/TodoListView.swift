            ...
            List(viewModel.todoList) { todoItem in
                NavigationLink(
                    ...
                    HStack {
                        ...
                        case .low:
                            Circle().fill(Color.green).frame(width: 40)
                        case .medium:
                            Circle().fill(Color.yellow).frame(width: 40)
                        case .high:
                            Circle().fill(Color.red).frame(width: 40)
                        }
                        VStack(alignment: .leading) {
                            Text(todoItem.title).font(.title)
                            Text(todoItem.startDate, style: .date).font(.subheadline)
                            Text(todoItem.startDate, style: .time).font(.subheadline)
                        }
                    }
                }
            }
